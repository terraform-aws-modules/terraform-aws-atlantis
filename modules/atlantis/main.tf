locals {
  ...
  secret_name_value_from = var.atlantis_github_user_token_ssm_parameter_name != "" ? data.aws_ssm_parameter.atlantis_github_user_token[0].arn : ""

  # webhook
  secret_webhook_key        = "ATLANTIS_GH_WEBHOOK_SECRET"
  secret_webhook_value_from = data.aws_ssm_parameter.atlantis_github_webhook_secret.arn
  ...
  container_definitions = var.custom_container_definitions == "" ? module.container_definition_github_gitlab.json_map_encoded_list : var.custom_container_definitions
  ...
  container_definition_environment = [
    ...
    },
    {
      ...
      value = var.atlantis_github_organization
    },
    {
    ...
    },
    {
      ...
      value = var.atlantis_github_app_key_file
    },
    {
    ...
    },
    {
      ...
      value = var.atlantis_repo_config_json != "" ? file(var.atlantis_repo_config_json) : ""
    },
    {
  ...
  container_definition_secrets_1 = local.secret_name_value_from != "" ? [
    {
  ...
  container_definition_secrets_2 = [
    {
      ...
      valueFrom = local.secret_webhook_value_from
    }
  ]
...
data "aws_ssm_parameter" "atlantis_github_user_token" {
  count = var.atlantis_github_user_token_ssm_parameter_name != "" ? 1 : 0

  name = var.atlantis_github_user_token_ssm_parameter_name
}

data "aws_ssm_parameter" "atlantis_github_webhook_secret" {
  name = var.atlantis_github_webhook_secret_ssm_parameter_name
}
...
resource "aws_lb_listener_rule" "unauthenticated_access_for_cidr_blocks" {
  ...
  condition {
    source_ip {
      values = sort(compact(concat(var.github_webhooks_cidr_blocks, var.whitelist_unauthenticated_cidr_blocks)))
    }
  }
}
...
module "alb_https_sg" {
  ...
  ingress_cidr_blocks = sort(compact(concat(var.github_webhooks_cidr_blocks, var.alb_ingress_cidr_blocks)))
...
data "aws_iam_policy_document" "ecs_task_access_secrets" {
  statement {
    ...
    resources = flatten(list(
      data.aws_ssm_parameter.atlantis_github_webhook_secret.*.arn,
      data.aws_ssm_parameter.atlantis_github_user_token.*.arn,
    ))
